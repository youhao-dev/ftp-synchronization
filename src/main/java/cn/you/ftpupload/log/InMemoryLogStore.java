...
public class InMemoryLogStore {
    ...
    /** 写入系统信息记录。 */
    public void info(String message) { addSystem(new SystemLogEntry(LocalDateTime.now(),"INFO",message)); }
    /** 写入系统警告记录。 */
    public void warn(String message) { addSystem(new SystemLogEntry(LocalDateTime.now(),"WARN",message)); }
    /** 写入系统错误记录。 */
    public void error(String message) { addSystem(new SystemLogEntry(LocalDateTime.now(),"ERROR",message)); }
    ...
    /** 添加系统记录监听器。 */
    public void addSystemListener(Consumer<SystemLogEntry> listener) { if (listener!=null) systemListeners.add(listener); }
    /** 添加 FTP 记录监听器。 */
    public void addFtpListener(Consumer<FtpLogEntry> listener) { if (listener!=null) ftpListeners.add(listener); }
