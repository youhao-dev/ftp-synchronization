...
public class FtpConnectionPool implements AutoCloseable {
    ...
    public Lease borrow() throws IOException {
        if (closed.get()) throw new IOException("FTP 连接池已关闭");
        ...
        while (true) {
            ...
            if (remain<=0) throw new IOException("FTP 连接池繁忙，等待连接超时");
            try {
                pooled=idle.poll(remain,TimeUnit.NANOSECONDS);
                if (pooled==null) throw new IOException("FTP 连接池繁忙，等待连接超时");
                if (prepare(pooled)) return new Lease(this,pooled);
                destroy(pooled);
            } catch (InterruptedException e) { Thread.currentThread().interrupt(); throw new IOException("等待 FTP 连接被中断",e); }
        }
    }
    ...
    private FTPClient createClient() throws IOException {
        ...
        if (settings.ftpHost().isBlank()) throw new IOException("未配置 FTP 地址");
        ...
        try {
            ...
            if (!FTPReply.isPositiveCompletion(ftp.getReplyCode())) throw new IOException("FTP 服务器拒绝连接: "+ftp.getReplyString());
            ftp.setSoTimeout(settings.readTimeoutMs());
            if (!ftp.login(settings.ftpUsername(),settings.ftpPassword())) throw new IOException("FTP 登录失败: "+ftp.getReplyString());
            ftp.enterLocalPassiveMode(); ftp.setFileTransferMode(FTP.STREAM_TRANSFER_MODE); ftp.setFileType(FTP.BINARY_FILE_TYPE);
            return ftp;
        } catch (Exception e) {
            disconnect(ftp);
            throw e instanceof IOException io?io:new IOException("FTP 连接失败",e);
        }
    }
