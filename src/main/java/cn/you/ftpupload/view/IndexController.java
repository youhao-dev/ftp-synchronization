...
import cn.you.ftpupload.FxStart;
import cn.you.ftpupload.config.AppSettings;
...
import cn.you.ftpupload.pojo.SystemLogEntry;
...
public class IndexController {
    ...
    @FXML
    private ComboBox<String> frequencyUnitCombo;
    @FXML
    ...
    private final FtpUploadStack uploadStack = appContext.uploadStack();
    ...
    private void initialize() {
        ...
        initFtpLogTable();
        initSettings();
        uploadStack.addListener(event -> Platform.runLater(() -> updateStatus(event)));
        logStore.addSystemListener(this::enqueueSystemLog);
        logStore.addFtpListener(this::enqueueFtpLog);
    ...
    private void initRulesTable() {
        ...
        transferTypeColumn.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().isDownload()?"下载":"上传"));
        ...
        rulesTable.setPlaceholder(new Label("暂无传输规则，点击右上角“新增规则”开始配置"));
    ...
    private void initSystemLogTable() {
        ...
        systemMessageColumn.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().message()));
        systemLogTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY_FLEX_LAST_COLUMN);
        systemLogTable.setPlaceholder(new Label("暂无系统传输记录"));
    ...
    private void initFtpLogTable() {
        ...
        ftpStatusColumn.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().status()));
        ...
        ftpMessageColumn.setCellValueFactory(data -> new SimpleStringProperty(data.getValue().message()));
        ftpLogTable.setPlaceholder(new Label("暂无 FTP 文件记录；上传或下载任务执行后会在这里实时显示最近 200 条"));
        ftpStatusColumn.setCellFactory(column -> new TableCell<>() {
            ...
            protected void updateItem(String item, boolean empty) {
                ...
                if (!empty && item != null)
                    getStyleClass().add("成功".equals(item) ? "ftp-success" : "失败".equals(item) ? "ftp-fail" : "ftp-skip");
            }
        });
    }
    ...
    private void initSettings() {
        frequencyUnitCombo.setItems(FXCollections.observableArrayList("秒", "分钟", "小时"));
        loadSettingsToForm();
    ...
    private void clearRules(ActionEvent event) {
        ...
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "确定清空全部传输规则吗？此操作会立即更新 pathRules.json。", ButtonType.CANCEL, ButtonType.OK);
        alert.setTitle("清空规则");
        alert.setHeaderText("确认清空全部规则");
        if (alert.showAndWait().orElse(ButtonType.CANCEL) == ButtonType.OK) {
            pathRulesService.clear();
            refreshData();
            logStore.warn("用户清空了全部传输规则");
        }
    }
    ...
    private void clearFtpLogs(ActionEvent event) {
        ...
        ftpLogItems.clear();
        statusLabel.setText("FTP 文件记录已清空");
    }
    ...
    private void saveSettings(ActionEvent event) {
        try {
            ...
            settingsStatusLabel.setText("已保存并立即生效");
            statusLabel.setText("运行配置已保存");
            refreshData();
            showInfo("保存成功", "FTP 与运行配置已保存，并已立即生效。");
        } catch (Exception e) {
            showWarning("保存配置失败", safeMessage(e));
        }
    }
    ...
    private void testConnection(ActionEvent event) {
        ...
        try {
            ...
            showWarning("配置有误", safeMessage(e));
            return;
        }

        testConnectionButton.setDisable(true);
        testConnectionButton.setText("连接中…");
        settingsStatusLabel.setText("正在测试 FTP 连接…");
        ...
        Thread.ofVirtual().name("ftp-test").start(() -> {
            try {
                ...
                logStore.info("FTP 连接测试成功：" + settings.ftpHost() + ":" + settings.ftpPort());

                Platform.runLater(() -> {
                    testConnectionButton.setDisable(false);
                    testConnectionButton.setText("测试连接");
                    settingsStatusLabel.setText(result);
                    showInfo("连接成功", "FTP 服务器连接成功。\n\n服务器：" + settings.ftpHost() + ":" + settings.ftpPort() + "\n用户：" + settings.ftpUsername());
                });
            } catch (Exception e) {
                String message = safeMessage(e);
                logStore.error("FTP 连接测试失败：" + message);

                Platform.runLater(() -> {
                    testConnectionButton.setDisable(false);
                    testConnectionButton.setText("测试连接");
                    settingsStatusLabel.setText("连接失败：" + message);
                    showWarning("FTP 连接失败", message);
                });
            }
        });
    }
    ...
    private void initStatusColumn() {
        ...
        statusColumn.setCellFactory(column -> new TableCell<>() {
            ...
            protected void updateItem(Boolean running, boolean empty) {
                ...
                setText(running ? "运行中" : "已停止");
                getStyleClass().add(running ? "status-running" : "status-stopped");
                setTooltip(new Tooltip(running ? "点击停止此规则" : "点击启动此规则"));
            }
    ...
    public void refreshData() {
        ...
        lastRunLabel.setText(uploadStack.getLastFinishedTime() == null ? "尚未执行" : uploadStack.getLastFinishedTime().format(TIME_FORMAT));
        poolStatusLabel.setText(appContext.ftpPool().totalCount() + " / " + appContext.config().ftpPoolSize());
        runningBadge.setText(uploadStack.isRunning() ? "运行中" : "等待中");
    ...
    private void ruleStatusChanged(FileInfo rule) {
        ...
        refreshData();
        String type=rule.isDownload()?"下载规则":"上传规则";
        if (rule.getIsRunning()) logStore.warn("启动"+type+"："+nvl(rule.getName()));
        else logStore.info("停止"+type+"："+nvl(rule.getName()));
    }
    ...
    private void initActionColumn() {
        actionColumn.setCellFactory(column -> new TableCell<>() {
            private final Button viewButton = createButton("查看", "table-btn");
            private final Button editButton = createButton("编辑", "table-btn", "table-btn-primary");
            private final Button deleteButton = createButton("删除", "table-btn", "table-btn-danger");
    ...
    private void openRuleDialog(AddController.Mode mode, FileInfo fileInfo) {
        try {
            FXMLLoader loader = new FXMLLoader(FxStart.class.getResource("/fxml/add.fxml"));
            ...
            stage.setTitle(mode == AddController.Mode.ADD ? "新增规则" : mode == AddController.Mode.EDIT ? "编辑规则" : "查看规则");
            stage.setScene(scene);
            stage.showAndWait();
        } catch (Exception e) {
            showError("打开规则窗口失败", e);
        }
    }
    ...
    private void deleteRule(FileInfo fileInfo) {
        ...
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "确定删除规则“" + nvl(fileInfo.getName()) + "”吗？", ButtonType.CANCEL, ButtonType.OK);
        alert.setTitle("删除规则");
        alert.setHeaderText(null);
        if (alert.showAndWait().orElse(ButtonType.CANCEL) == ButtonType.OK) {
            pathRulesService.deletePathById(fileInfo.getId());
            refreshData();
            logStore.warn("删除"+(fileInfo.isDownload()?"下载规则":"上传规则")+"："+nvl(fileInfo.getName()));
        }
    }
     ...
     * 处理调度器状态事件并刷新顶部状态。
     */
    private void updateStatus(FtpUploadStack.UploadEvent event) {
        statusLabel.setText(event.time().format(DateTimeFormatter.ofPattern("HH:mm:ss")) + "  " + event.message());
        refreshData();
    }
    ...
    private void loadSettingsToForm() {
        ...
        settingsStatusLabel.setText("配置修改后点击“保存设置”立即生效");
    }
    ...
    private AppSettings buildSettingsFromForm() {
        ...
        if (host.isBlank()) throw new IllegalArgumentException("FTP 地址不能为空");
        int port = parseInt(ftpPortField, "FTP 端口", 1, 65535), connect = parseInt(connectTimeoutField, "连接超时", 1_000, 120_000), read = parseInt(readTimeoutField, "读取超时", 1_000, 300_000), data = parseInt(dataTimeoutField, "数据超时", 1_000, 300_000);
        int poolSize = parseInt(poolSizeField, "连接池大小", 1, 16), borrow = parseInt(poolBorrowTimeoutField, "连接池等待超时", 1_000, 120_000);
        int interval = frequencySeconds(), scheduleSecond = parseInt(scheduleSecondField, "一分钟对齐秒数", 0, 59), latest = parseInt(latestFileCountField, "每次检查文件数", 1, 100), stable = parseInt(fileStableSecondsField, "文件稳定时间", 0, 3_600);
        return new AppSettings(host, port, username, password, connect, read, data, poolSize, borrow, interval, scheduleSecond, latest, stable);
    }
    ...
    private int frequencySeconds() {
        int value = parseInt(frequencyValueField, "执行频率", 1, 86_400);
        String unit = frequencyUnitCombo.getValue() == null ? "秒" : frequencyUnitCombo.getValue();
        long seconds = (long) value * ("小时".equals(unit) ? 3600 : "分钟".equals(unit) ? 60 : 1);
        if (seconds < 5 || seconds > 86_400) throw new IllegalArgumentException("执行频率换算后必须在 5 秒~24 小时");
        return (int) seconds;
    }
    ...
    private int parseInt(TextField field, String name, int min, int max) {
        ...
        try {
            ...
            if (number < min || number > max) throw new IllegalArgumentException(name + "必须在 " + min + "~" + max);
            return number;
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException(name + "必须填写整数");
        }
    }
